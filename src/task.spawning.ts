...
function spawnNewCreepsForRoom(spawnName:string){
    ...
    if(Game.spawns[spawnName].room.find(FIND_HOSTILE_CREEPS).length>0){
        ...
        if (Game.spawns[spawnName].room.energyCapacityAvailable>=1800){
            if(     checkThenSpawn(spawnName,ROLE_MINER,CASTE_WORKER,1,L5_1800_Miner,energy)){}
            else if(checkThenSpawn(spawnName,ROLE_COURIER,CASTE_WORKER,1,L5_1800_Courier,energy)){}
            else if(checkThenSpawn(spawnName,ROLE_PAVER,CASTE_WORKER,1,L5_1800_Worker,energy)){}
            else if(checkThenSpawn(spawnName,ROLE_MINER,CASTE_WORKER,2,L5_1800_Miner,energy)){}
            else if(checkThenSpawn(spawnName,ROLE_PAVER,CASTE_WORKER,3,L5_1800_Worker,energy)){}
        } else 
        if (Game.spawns[spawnName].room.energyCapacityAvailable>=1300){
            ...
            else if(checkThenSpawn(spawnName,ROLE_PAVER,CASTE_WORKER,5,L4_1300_Worker,energy)){}
        } else 
